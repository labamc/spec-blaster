...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      if (now > g.nextCapyMsg && !g.capyMsg) {
      ...
      if (!g.boss || g.endless) {
        ...
        if (now - g.lastWord > interval) {
          ...
          // Assign enemy behavior based on level
    ...
    function loseLife(g: GState, now: number) {
      ...
      // scatter particles from player position
      for (let i = 0; i < 12; i++)
    ...
    <main style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:"100vh", padding:"0.5rem", background:"#0d0d14" }}>
      <div style={{ width:"100%", maxWidth:"800px" }}>
        ...
        <div style={{ marginTop:"0.4rem", display:"flex", justifyContent:"space-between", fontSize:"0.65rem", padding:"0 2px" }}>
          <span style={{ color:"rgba(255,255,255,0.2)", fontFamily:"monospace" }}>← → / A D move · SPACE shoot</span>
...
function draw(ctx: CanvasRenderingContext2D, g: GState, cw: number, now: number) {
  ...
  g.words.forEach(w => {
    ...
    // chargers get a subtle red tint as warning
    ctx.fillStyle = w.beh === "charge" && w.type !== "powerup" ? "#fca5a5" : col
  ...
  if (g.boss) {
    ...
    ctx.save()
    ctx.shadowColor = b.color; ctx.shadowBlur = 20 * pulse
  ...
  g.bullets.forEach(b => {
    if (!b.enemy) {
      ctx.fillStyle = g.upgrades.spray ? "#22d3ee" : (g.triple || g.upgrades.triple) ? "#4ade80" : "#966bec"
  ...
  // combo display
  if (g.combo >= 3) {
    const comboAlpha = Math.min(1, (now - g.lastKill + 200) / 300)
  ...
  if (g.triple || g.upgrades.triple) { ctx.fillStyle = "#4ade80"; ctx.fillText("ENGAGE", cw-10, pwY); pwY += 13 }
